...
namespace xml
{
    ...
    public class ThintaXMLConfig
    {
		...
		private static ThintaXMLConfig thisConfig = null;

		/// <summary>
		/// 以默认配置文件名，初始化配置文件类。默认配置文件名为 config.xml
		/// </summary>
		public ThintaXMLConfig()
			: this("config.xml")
		{
		}
		...
		public ThintaXMLConfig(string FileName)
		{
			ds = new DataSet();
			ConfigFile = FileName;
		}
		...
		/// 默认的配置文件
		/// </summary>
        public static ThintaXMLConfig DefaultConfig
		{
			get
			{
				if (thisConfig == null)
                    thisConfig = new ThintaXMLConfig();
